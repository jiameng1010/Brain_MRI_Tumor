...
class SamsegTumor(Samseg):
    ...
    def estimateModelParameters( self, initialBiasFieldCoefficients=None, initialDeformation=None,
        ...
        for multiResolutionLevel in range(numberOfMultiResolutionLevels):
            ...
            for iterationNumber in range(maximumNumberOfIterations):
                ...
                while True:
                    print('EMIterationNumber=%d', EMIterationNumber)
    ...
    def writeResults(self, biasFields, posteriors, posteriorsTumor):
        ...
        structureNumbers = np.array(np.argmax(posteriors[:, :-1], 1), dtype=np.uint32)
        structureNumbersT = np.array(np.argmax(posteriors[:, :], 1), dtype=np.uint32)

        segmentation = np.zeros(self.imageBuffers.shape[:3], dtype=np.int32)
        segmentationT = np.zeros(self.imageBuffers.shape[:3], dtype=np.int32)
        ...
        segmentationT[self.mask] = fslabels[structureNumbersT]
        ...
        self.writeImage(segmentationT, os.path.join(self.savePath, 'segT.mgz'), saveLabels=True)
