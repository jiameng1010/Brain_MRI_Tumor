...
for entry in os.scandir(training_data_output_dir):
    ...
    if not output_dir == '/media/mjia/Seagate Backup Plus Drive/Researches/Volume_Segmentation/my_training_data/00026':
        continue
    ...
    #mlab.triangular_mesh(mindboggle_whole_brain_v[:, 0], mindboggle_whole_brain_v[:, 1], mindboggle_whole_brain_v[:, 2], mindboggle_whole_brain.faces, color=(1, 0, 0), opacity=0.2)
    ...
    mlab.triangular_mesh(tumor_mesh_vertices[:, 0], tumor_mesh_vertices[:, 1], tumor_mesh_vertices[:, 2], tumor_mesh.faces, color=(0, 1, 1),
                         opacity=0.2)
    mlab.points3d(tumor_PC[0][:, 0], tumor_PC[0][:, 1], tumor_PC[0][:, 2], color=(1, 0, 0), scale_factor=2)
    #mlab.points3d(brain_pc[0][:, 0], brain_pc[0][:, 1], brain_pc[0][:, 2], color=(0, 1, 0), scale_factor=2)
