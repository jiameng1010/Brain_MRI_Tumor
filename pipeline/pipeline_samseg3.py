...
class pipeline():
    def __init__(self, inputdir, output_folder, num_points, is_synthesised, resume=False, use_BraTS_label=False):
    ...
    def samseg_def(self, input_file, output_folder, use_GT_disp=False):
        ...
        subprocess.run(['cp', SAMSEG_ATLAS_DIR + '/compressionLookupTable.txt', output_folder + '/atlas'])
        subprocess.run(['cp', SAMSEG_ATLAS_DIR + '/sharedGMMParameters.txt', output_folder + '/atlas'])
        subprocess.run(['cp', SAMSEG_ATLAS_DIR + '/template.nii', output_folder + '/atlas'])

        if use_GT_disp:
            displacement, _ = load(self.input_dir + '/SimTumor_def_inverse.mha')
            displacement = displacement
            displacement_inv, _ = load(self.input_dir + '/SimTumor_def.mha')
            displacement_inv = displacement_inv
        else:
            displacement = self.load_pred_disp()
        ...
        deform_atlas_vol(SAMSEG_ATLAS_DIR + '/template.nii')
        deform_atlas(SAMSEG_ATLAS_DIR + '/atlasForAffineRegistration.txt.gz')
        deform_atlas(SAMSEG_ATLAS_DIR + '/atlas_level1.txt.gz')
        deform_atlas(SAMSEG_ATLAS_DIR + '/atlas_level2.txt.gz')
    ...
    def run_samseg(self):
        self.samseg_def(self.t1, self.input_dir + '/samseg3', use_GT_disp=False)
        self.project_samseg(src=self.input_dir + '/samseg3/seg.mgz', dist=self.output_dir + '/samseg3_010.nii.gz',
                            direct_copy=True)
...
if __name__ == "__main__":
    ...
    pp = pipeline(config.input_folder, config.input_folder+'/'+config.output_folder, config.num_points,
                  is_synthesised=config.is_synthesised, resume=True)
    pp.run_samseg()
